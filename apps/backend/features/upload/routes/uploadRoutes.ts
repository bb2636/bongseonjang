  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (profile.requiresAuth && !req.userId) {
        return res
          .status(401)
          .json({ error: "Authentication required for this upload type" });
      }
...
const ALLOWED_REVIEW_MIME_TYPES = [
  "image/jpeg",
  "image/png",
  "image/gif",
  "image/webp",
...
router.get("/objects/*objectPath", async (req, res) => {
  ...
  const objectPath = Array.isArray(objectPathArr)
    ? objectPathArr.join("/")
    : objectPathArr;
  ...
  try {
    await objectStorageService.downloadObjectByPath(
      `/objects/${objectPath}`,
      res,
    );
