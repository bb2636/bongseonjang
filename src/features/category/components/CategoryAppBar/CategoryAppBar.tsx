...
export default function CategoryAppBar({ 
  onCartClick, 
  onLogoClick,
  cartItemCount = 0,
}: CategoryAppBarProps) {
  return (
    <header className="category-app-bar">
      <button 
        type="button"
        className="category-app-bar__logo-button"
        onClick={onLogoClick}
      >
        <span className="category-app-bar__title">봉선장</span>
      </button>
      <button 
        className="category-app-bar__cart-button" 
        onClick={onCartClick}
        aria-label="장바구니"
      >
        <svg 
          className="category-app-bar__cart-icon" 
          width="26" 
          height="26" 
          viewBox="0 0 26 26" 
          fill="none"
        ...
        {cartItemCount > 0 && (
          <span className="category-app-bar__cart-badge">{cartItemCount}</span>
        )}
      </button>
    </header>
  );
